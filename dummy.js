...
<div class="price-cart">
<h3 id="updatedd_${datas[0]._id}">₹${datas[0].price} </h3>
<h5 id="quantityy_${datas[0]._id}">Quantity:${cartArray[i].item}</h5>
     ...
     cartmain.addEventListener("click", (e) => {
      ...
      let checkoutsubtotall= document.querySelector(".checkout_subtotal")
      let updatePricee=document.querySelector(`#updatedd_${data[0]._id}`)
      let checkoutQuatityy=document.querySelector(`#quantityy_${data[0]._id}`)
      sum1=0;
      total1=0
      for(let i=0;i<cartArray.length;i++){
      ...
      if (e.target.id == `minu_${datas[0]._id}`) {
        ...
        updatePricee.innerText=`₹${data[0].price * cartArray[i].item}`
        ...
        checkoutsubtotall.innerText=`₹ ${total1}`
